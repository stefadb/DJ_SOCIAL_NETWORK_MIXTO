...
function BranoTableRow(props: {element: { numero_passaggi: number; id_brano_1: number; brano_1_array: BranoDb[] } | { numero_passaggi: number; id_brano_2: number; brano_2_array: BranoDb[] }, getNomiArtistiBrano: (id: number) => Promise<ArtistaDb[]>}) {
    const [artistiBrano, setArtistiBrano] = useState<ArtistaDb[] | null>(null);
    
    async function loadArtistiPassaggi() {
        const nomi = await props.getNomiArtistiBrano("id_brano_2" in props.element ? props.element.id_brano_2 : props.element.id_brano_1);
